...
public class LeshanClientDemo {
    ...
    private final static String[] modelPaths = new String[] { "3311.xml", "3342.xml" , "3343.xml" };

    private static final int OBJECT_ID_TEMPERATURE_SENSOR = 3303;
    private final static String DEFAULT_ENDPOINT = "LeshanClientDemo";
    private final static String USAGE = "java -jar leshan-client-demo.jar [OPTION]\n\n";
    private final static int SHORT_SERVER_ID = 1234575;
    ...
    public static void createAndStartClient(String endpoint, String localAddress, int localPort, boolean needBootstrap,
        ...
        if (needBootstrap) {
            if (pskIdentity != null) {
                ...
                initializer.setInstancesForObject(SERVER, new Server(SHORT_SERVER_ID, 30, BindingMode.U, false));
            } else {
            ...
            if (pskIdentity != null) {
                initializer.setInstancesForObject(SECURITY, psk(serverURI, SHORT_SERVER_ID, pskIdentity, pskKey));
                initializer.setInstancesForObject(SERVER, new Server(SHORT_SERVER_ID, 30, BindingMode.U, false));
            } else if (clientPublicKey != null) {
                initializer.setInstancesForObject(SECURITY, rpk(serverURI, SHORT_SERVER_ID, clientPublicKey.getEncoded(),
                        clientPrivateKey.getEncoded(), serverPublicKey.getEncoded()));
                initializer.setInstancesForObject(SERVER, new Server(SHORT_SERVER_ID, 30, BindingMode.U, false));
            } else if (clientCertificate != null) {
                initializer.setInstancesForObject(SECURITY, x509(serverURI, SHORT_SERVER_ID, clientCertificate.getEncoded(),
                        clientPrivateKey.getEncoded(), serverCertificate.getEncoded()));
                initializer.setInstancesForObject(SERVER, new Server(SHORT_SERVER_ID, 30, BindingMode.U, false));
            } else {
                initializer.setInstancesForObject(SECURITY, noSec(serverURI, SHORT_SERVER_ID));
                initializer.setInstancesForObject(SERVER, new Server(SHORT_SERVER_ID, 30, BindingMode.U, false));
            }
        }
        initializer.setInstancesForObject(DEVICE, new MyDevice());
        initializer.setInstancesForObject(LOCATION, locationInstance);
        initializer.setInstancesForObject(3311, new OnOffSensor());
        initializer.setInstancesForObject(3343, new BrightnessControl());
